...
const WIDTH = 700;
const HEIGHT = 140;
export const COMIC_BASE_COLOR = 0xFFD700;
export const COMIC_ACTIVE_COLOR = 0xFFFFFF;

export const COMIC_TEXT_STYLE = {
...
export class ComicCaptionDecoder implements CaptionDecoder {
	public readonly type = CaptionType.COMIC;
	...
	public applyStyles(world: World, entity: Entity): boolean {
		...
		stroke.set(StrokeStyle, { width: 7, join: StrokeJoin.ROUND, cap: StrokeCap.ROUND });
		appendChild(world, stroke, entity);

		// 3D hard shadow (0 blur)
		...
		shadow.add(Blur);
		shadow.set(Blur, { value: 0 });
		shadow.add(Offset);
		shadow.set(Offset, { x: 5, y: 6 });
		appendChild(world, shadow, entity);

		loadWebFont(world, COMIC_TEXT_STYLE.fontFamily);
		return true;
	}
	...
	public seekTo(world: World, entity: Entity, relativeTime: number): void {
		...
		if (groupIndex !== this.currentGroupIndex || wordIndex !== this.currentWordIndex) {
			...
			if (wordIndex !== -1) {
				...
				range.add(Color);
				range.set(Color, { value: COMIC_ACTIVE_COLOR });
				...
				fill.add(Color);
				fill.set(Color, { value: COMIC_ACTIVE_COLOR });
	...
	public draw(world: World, entity: Entity): void {
		...
		const activeColor = colors?.[0] ?? COMIC_ACTIVE_COLOR;
